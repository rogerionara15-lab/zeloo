...
const PaymentSuccess: React.FC<PaymentSuccessProps> = ({ planName, paymentStatus, onContinue }) => {
      ...
      <div className="max-w-2xl w-full bg-white rounded-[4rem] p-12 md:p-20 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)] border border-slate-100 relative overflow-hidden text-center">
        ...
        <div className="relative z-10">
          ...
          <p className="text-xl text-slate-500 font-medium leading-relaxed mb-12 max-w-lg mx-auto">
            {isAwaiting ? (
              <>Recebemos seu sinal de pagamento via PIX/Boleto. Agora, crie suas credenciais de acesso para que possamos validar seu comprovante no portal administrativo e liberar seu plano <span className="text-slate-900 font-black">{planName}</span>.</>
            ) : (
          ...
          <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 mb-12 flex flex-col md:flex-row items-center justify-between gap-6">
